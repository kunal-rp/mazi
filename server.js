...
function newConnection(socket){
    ...
    connection.query(query,function(err, results){
        if(err){
            ...
            for(i=0;i < results.length; i++){
                array.push(results[i]['college_id']);
                data[results[i]['college_id']] = results[i]['college_version'];
            }
            data['ids'] = array;
    ...
    socket.on('setUser',function(data){
        ...
        clients[user_id] = {socket_id : socket.id};
    });
            ...
            if(err){
               ...
               initial_data['college_data'] = final;
                ...
                if(err2){
                    ...
                    final2['ids'] = array2;
                    initial_data['parking_data'] = final2;
                    socket.emit('allData',initial_data);
                    console.log('allData');
                    console.log(initial_data);
                    }
                });
    ...
    socket.on('register', function(data){
        ...
        clients[user_id]['status'] = "looking"; 
        ...
        connection.query(query_1, function(err_1,results_1){
            if(err_1){
                ...
                if(results_1.length == 0){
                    ...
                    connection_data['index_timestamp_room'] = connection_data['start_timestamp']+connection_data['room_name'];
