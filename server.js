...
var table_suggestion = 'suggestions_bugs'
...
app.get('/checkUser',function (req,res) {
	...
	if(req.get("token") == undefined ||req.get("user_type") == undefined || codes[req.get("user_type")] == undefined){
		...
		jwt.verify(req.get("token"), codes[req.get("user_type")], function(err, decoded) {
			if(err){
				console.log(codes[req.get("user_type")])
				res.json({code:0 , message:"Error Authenticating Token"})
			}
			else{
													...
													if(clients[results[0]['user_id']] != undefined && clients[results[0]['user_id']]['socket_id'] != undefined){
														...
														if(results[0].email_verified == 1){
															generateNewPassword(20,function(data){
																...
																console.log(results[0]['user_id'] + "| "+ clients[results[0]['user_id']]['auth_code'])
						...
						if(decoded.user_id == undefined ||decoded.type == undefined ||decoded.system_data == undefined ||decoded.comment == undefined ){
							...
							connectionPool.getConnection(function(connection_error,connection){
								if(connection_error){
									...
									var query_insert_suggestion = "INSERT INTO "+table_suggestion + "(`timestamp`, `user_id`, `type`, `system_data`, `message`) VALUES("+Math.round((new Date()).getTime() / 1000)+",'"+user_id+"',"+mysql.escape(type)+","+mysql.escape(system_data)+","+mysql.escape(comment)+")";
									connection.query(query_insert_suggestion,function(err, results){
										if(err){
											console.log("Error/checkUser API  :" + err);
											console.log("Query : " + query_insert_suggestion);
											res.json({code:0,message:"Error with DB Query"});
										}
										else{
											res.json({code:1});
			...
			if(err_verify_user){
				...
				jwt.verify(req.get("token_data"), clients[decoded_verify_user.user_id].auth_code, function(err, decoded) {
					if(err){
						...
						if(decoded.new_user_password == undefined || decoded.new_user_name == undefined ||decoded.user_password == undefined ||decoded.user_email == undefined || decoded.user_id == undefined){
							res.json({code: 0, message: "Parameters not meet"});
						}
						else{
							var user_id = decoded.user_id
...
app.get('/createUser',function (req,res) {
	...
	if(req.get("token") == undefined ||req.get("user_type") == undefined || codes[req.get("user_type")] == undefined){
		res.json({code: 0, message: "Parameters not meet 1"});
	}
	else{
		jwt.verify(req.get("token"), codes[req.get("user_type")], function(err, decoded) {
			if(err){
				...
				var data = {}
				if(decoded.user_name == undefined ||decoded.user_password == undefined ||decoded.user_email == undefined  ){
					res.json({code: 0, message: "Parameters not meet 2"});
				}
				else{
...
function newConnection(socket){
	...
	socket.on('setUser',function(data){
		...
		if(clients[socket.user_id] == undefined  || clients[socket.user_id]['status'] == undefined ){
...
socket.on('joinRoom',function(data){
	...
	if(clients[mirror_user_id].connected == false){
		userHasDisconnected(mirror_user_id);
	}
});
...
socket.on('cancelMatch',function(){
	var rider_user_id = clients[socket.user_id].rider_user_id;
	var parker_user_id = clients[socket.user_id].parker_user_id;
	endMatch(rider_user_id,parker_user_id,2+"|"+socket.user_id);
...
function userHasDisconnected(user_id){
	...
	clients[user_id]['timer'] = setTimeout(function(){
		...
		endMatch(clients[user_id].rider_user_id,clients[user_id].parker_user_id,1+"|"+user_id)
		delete clients[user_id];
